...
class ApiClient(
    ...
    suspend fun pushChanges(changes: List<SyncChange>, lastSyncAt: Long): ApiResponse<List<SyncConfirmation>> {
        return client.post("$baseUrl/api/v1/sync") {
            setBody(SyncRequest(changes, lastSyncAt))
        }.body()
    }
}
```
...
class SyncEngine(
    ...
    suspend fun sync(): SyncResult {
        ...
        val response = apiClient.pushChanges(pending, lastSync)
        
        // 4. Update status
        response.data?.forEach { confirmation ->
            database.markSynced(confirmation.localId, confirmation.remoteId, confirmation.version)
        }
    ...
    private suspend fun mergeChange(remote: SyncChange) {
        val local = database.getByRemoteId(remote.entityId)
        when {
